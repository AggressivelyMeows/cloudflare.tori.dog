<template>
  <div class="font-medium z-10">
    ...
    <div class="space-y-3 grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
      <div class="tori-card" v-for="post in storybooks.filter(x => x.category === 'workers')" :key="post.storybook">
        <NuxtLink :to="`/labs/${post?.pages?.[0]?.path}`" class="text-primary-500 mb-1 text-2xl">
