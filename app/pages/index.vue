  ...
  <div class="font-medium z-10">
    ...
    <div class="tori-card mt-6">
      <p class="text-sm text-stone-300">
        Cloudflare offers a powerful set of serverless tools that allow developers to build and deploy applications at the edge of the network.
        <br/><br/>
        Our goal with this site is to provide a comprehensive set of tutorials and resources to help developers get started with Cloudflare's serverless platform, including Workers, Durable Objects, R2, and more!
      </p>
    </div>
    ...
    <div class="space-y-3 grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
      <div class="tori-card" v-for="post in postsSorted.filter(x => x.meta.category === 'Workers' && x.meta.type !== 'next-steps')" :key="post.path">
        <NuxtLink :to="`/labs${post.path}`" class="text-primary-500 mb-1 text-2xl">
...
const postsSorted = computed(() => { 
  ...
  return posts.value.sort((a, b) => {
    // @ts-expect-error - meta exists
    const dateA = new Date(a.meta.created || '').getTime()
    // @ts-expect-error - meta exists
    const dateB = new Date(b.meta.created || '').getTime()

    return dateB - dateA
  }).filter(x => {
    if (x.meta.status === 'hidden') return false
    return true
  })
})
